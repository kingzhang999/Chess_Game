...
public class Cars_B_Movement implements MoveBehavior {
    private int x;
    private int y;
    private JButton nowPosition;
    private final ArrayList<JButton> chess_B_BlockList_moment_car = new ArrayList<>();
    Car car;

    public Cars_B_Movement(Car car) {
        this.car = car;
        updateLocation();
    }
    ...
    public boolean move(JButton target_chess_block) {
        ...
        if(car.getChoiceState() == AbstractChessPiece.ChoiceState.CHOICE_ABLE){
            if(BlackPlayer.isInBlackPieceList(car)){
    ...
    private boolean real_B_move(JButton target_chess_block){
        ...
        if(target_x < ChessBoard.ROWS &&//防止棋子出界并判断下一个格子是否可以移动。
                chess_B_BlockList_moment_car.contains(ChessBoard.getChessBoardElement(target_x,target_y))
            ...
            if(targetImageIcon == ChessBoard.WHITE){//如要去的格子是白格子，则把要去的格子的背景换成相应棋子在白格子下的背景。
                ...
                ChessBoard.changeChessBoard(target_x,target_y,ChessBoard.BLACK_CAR_W);

            }else {//不然则把要去的格子的背景换成相应棋子在黑格子下的背景。

                ChessBoard.changeChessBoard(target_x,target_y,ChessBoard.BLACK_CAR_B);
    ...
    private void updatePieceList(){
        if(BlackPlayer.isInBlackPieceList(car)){
            ...
            chess_B_BlockList_moment_car.clear();//在每一次排查完合适的格子之后，删除原来的列表。以防止之前过时的格子还能生效。
            chess_B_BlockList_moment_car.addAll(List.of(scan_B_ChessBlock_canWalk()));//更新白棋可以移动的格子列表。
            car.getAttackBehavior().scan_B_canAttack();//更新白棋可以攻击的格子列表。//攻击行为暂时还没有实现。
        }
    }
}
