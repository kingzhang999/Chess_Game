...
public class Elephants_B_AttackBehaviors implements AttackBehavior {
    private int x;
    private int y;
    ...
    public Elephants_B_AttackBehaviors(Elephant elephant){
    ...
    public boolean attack(JButton being_attacked_chess_block) {
        ...
        if(attack_able_B_blocks.contains(being_attacked_chess_block)){
            ...
            if(targetImageIcon.getBackground() == ChessBoard.BackGroundType.WhiteBack){//如要去的格子是白格子，则把要去的格子的背景换成相应棋子在白格子下的背景。
                ...
                ChessBoard.changeChessBoard(x_future,y_future,ChessBoard.BLACK_ELEPHANT_W);

            }else {//不然则把要去的格子的背景换成相应棋子在黑格子下的背景。

                ChessBoard.changeChessBoard(x_future,y_future,ChessBoard.BLACK_ELEPHANT_B);

            }
            ChessBoard.getChessBoardElement(x_future,y_future).repaint();//重绘移动完后格子的贴图。
            ((Elephants_B_Movement)elephant.getMoveBehavior()).updateLocation();//更新移动行为中棋子的所在位置。
            scan_B_canAttack();//刷新可攻击的位置。
            return true;
        }
        return false;
