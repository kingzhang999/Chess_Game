...
public class Horses_W_Movement implements MoveBehavior {
    private int x;
    private int y;
    private JButton nowPosition;
    private final ArrayList<JButton> chess_W_BlockList_moment_horse = new ArrayList<>();
    Horse horse;

    public Horses_W_Movement(Horse horse){
        this.horse = horse;
        updateLocation();
    }
    ...
    public boolean move(JButton target_chess_block) {
        ...
        if(horse.getChoiceState() == AbstractChessPiece.ChoiceState.CHOICE_ABLE){
            if(WhitePlayer.isInWhitePiecesList(horse)){
    ...
    private boolean real_W_move(JButton target_chess_block) {
        ...
        if(//防止棋子出界并判断下一个格子是否可以移动。
            ...
            if(targetImageIcon == ChessBoard.WHITE){//如要去的格子是白格子，则把要去的格子的背景换成相应棋子在白格子下的背景。
                ...
                ChessBoard.changeChessBoard(target_x,target_y,ChessBoard.WHITE_HORSE_W);

            }else {//不然则把要去的格子的背景换成相应棋子在黑格子下的背景。

                ChessBoard.changeChessBoard(target_x,target_y,ChessBoard.WHITE_HORSE_B);
    ...
    public JButton[] scan_W_ChessBlock_canWalk() {
        ...
        /*for (JButton chess_blocks : chessBlockList_official){
            ChessBoard.findElement_test(ChessBoard.board,chess_blocks,"house_W_move");//测试能否找到元素。
        }*/
