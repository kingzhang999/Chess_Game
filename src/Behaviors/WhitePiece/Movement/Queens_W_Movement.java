...
public class Queens_W_Movement implements MoveBehavior {
    private int x;
    private int y;
    private JButton nowPosition;
    private final ArrayList<JButton> chess_W_BlockList_moment_queen = new ArrayList<>();
    Queen queen;

    public Queens_W_Movement(Queen queen) {
        this.queen = queen;
        updateLocation();
    }
    ...
    public boolean move(JButton target_chess_block) {
        ...
        if(queen.getChoiceState() == AbstractChessPiece.ChoiceState.CHOICE_ABLE){
            if(WhitePlayer.isInWhitePiecesList(queen)){
    ...
    public JButton[] scan_W_ChessBlock_canWalk() {
        ...
        x_copy = x;//复制x的值，以防止在循环中修改原值。
        y_copy = y;//复制y的值，以防止在循环中修改原值。
        ...
        for (;x_copy - 1 >= 0 && y_copy - 1 >= 0;x_copy--,y_copy--){//把x_copy放在这里的原因是每一次检查完之后，无论格子是否符合要求，都要把x_copy加一以检查下一个格子。
            if (!ChessBoard.hasPiece(ChessBoard.board[x_copy-1][y_copy-1])){
                chess_W_BlockList_moments.add(ChessBoard.board[x_copy-1][y_copy-1]);
    ...
    private boolean real_W_move(JButton target_chess_block){
        ...
        if(target_x < ChessBoard.ROWS &&//防止棋子出界并判断下一个格子是否可以移动。
                chess_W_BlockList_moment_queen.contains(ChessBoard.getChessBoardElement(target_x,target_y))
            ...
            queen.setChess_block_iconImage(targetImageIcon);//将车将要去的方块目前的状况记录下来。
            //将棋子所绑定的格子替换为目标格子。
            queen.setChess_block(target_chess_block);


            if(targetImageIcon == ChessBoard.WHITE){//如要去的格子是白格子，则把要去的格子的背景换成相应棋子在白格子下的背景。

                ChessBoard.changeChessBoard(target_x,target_y,ChessBoard.WHITE_QUEEN_W);

            }else {//不然则把要去的格子的背景换成相应棋子在黑格子下的背景。

                ChessBoard.changeChessBoard(target_x,target_y,ChessBoard.WHITE_QUEEN_B);
    ...
    private void updatePieceList(){
        if(WhitePlayer.isInWhitePiecesList(queen)){
            ...
            chess_W_BlockList_moment_queen.clear();//在每一次排查完合适的格子之后，删除原来的列表。以防止之前过时的格子还能生效。
            chess_W_BlockList_moment_queen.addAll(List.of(scan_W_ChessBlock_canWalk()));//更新白棋可以移动的格子列表。
            queen.getAttackBehavior().scan_W_canAttack();//更新白棋可以攻击的格子列表。
        }
    }
}
