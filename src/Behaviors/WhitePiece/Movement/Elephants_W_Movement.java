...
public class Elephants_W_Movement implements MoveBehavior {
    private int x;
    private int y;
    private JButton nowPosition;
    private final ArrayList<JButton> chess_W_BlockList_moment_elephant = new ArrayList<>();
    Elephant elephant;

    public Elephants_W_Movement(Elephant elephant){
    ...
    public boolean move(JButton target_chess_block) {
        ...
        if(elephant.getChoiceState() == AbstractChessPiece.ChoiceState.CHOICE_ABLE){
            if(WhitePlayer.isInWhitePiecesList(elephant)){
    ...
    private boolean real_W_move(JButton target_chess_block){
        ...
        if(target_x < ChessBoard.ROWS &&//防止棋子出界并判断下一个格子是否可以移动。
                chess_W_BlockList_moment_elephant.contains(ChessBoard.getChessBoardElement(target_x,target_y))
            ...
            if(targetImageIcon == ChessBoard.WHITE){//如要去的格子是白格子，则把要去的格子的背景换成相应棋子在白格子下的背景。
                ...
                ChessBoard.changeChessBoard(target_x,target_y,ChessBoard.WHITE_ELEPHANT_W);

            }else {//不然则把要去的格子的背景换成相应棋子在黑格子下的背景。

                ChessBoard.changeChessBoard(target_x,target_y,ChessBoard.WHITE_ELEPHANT_B);
    ...
    private void updatePieceList(){
        if(WhitePlayer.isInWhitePiecesList(elephant)){
            ...
            chess_W_BlockList_moment_elephant.clear();//在每一次排查完合适的格子之后，删除原来的列表。以防止之前过时的格子还能生效。
            chess_W_BlockList_moment_elephant.addAll(List.of(scan_W_ChessBlock_canWalk()));//更新白棋可以移动的格子列表。
            elephant.getAttackBehavior().scan_W_canAttack();//更新白棋可以攻击的格子列表。
        }
    }
}
