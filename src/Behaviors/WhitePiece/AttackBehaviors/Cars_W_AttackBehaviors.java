...
public class Cars_W_AttackBehaviors implements AttackBehavior {
    private int x;
    private int y;
    ...
    public Cars_W_AttackBehaviors(Car car) {
        this.car = car;
        updateLocation();
    }
    ...
    public boolean attack(JButton being_attacked_chess_block) {
        ...
        if(attack_able_W_blocks.contains(being_attacked_chess_block)){
            ...
            if(targetImageIcon.getBackground() == ChessBoard.BackGroundType.WhiteBack){//如要去的格子是白格子，则把要去的格子的背景换成相应棋子在白格子下的背景。
                ...
                ChessBoard.changeChessBoard(x_future,y_future,ChessBoard.WHITE_CAR_W);

            }else {//不然则把要去的格子的背景换成相应棋子在黑格子下的背景。

                ChessBoard.changeChessBoard(x_future,y_future,ChessBoard.WHITE_CAR_B);

            }
            ChessBoard.getChessBoardElement(x_future,y_future).repaint();//重绘移动完后格子的贴图。
            ((Cars_W_Movement)car.getMoveBehavior()).updateLocation();//更新移动行为中棋子的所在位置。
            scan_W_canAttack();//刷新可攻击的位置。
            return true;
        }
        return false;
